...
```{r}
# secure access of api token (api_token is in .gitignore)
...
# Using the Data: Demonstrations


```{r}
drugs <- read_csv("drugs_tbl.csv")


drugs |>
  mutate(drugName = fct_lump_n(drugName, 100)) |>
  group_by(patientSex) |>
  summarize(n = n()) |>
  ggplot(aes(x = "", y = n, fill= patientSex)) +
